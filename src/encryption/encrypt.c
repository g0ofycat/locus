...
size_t encrypt_data(const uint8_t *key, const void *src, size_t srcSize, void *dst, size_t dstCapacity) {
	...
	BCryptGenRandom(NULL, iv, AES_IV_SIZE, BCRYPT_USE_SYSTEM_PREFERRED_RNG);

	BCRYPT_KEY_HANDLE hkey;
	BCryptGenerateSymmetricKey(get_alg(), &hkey, NULL, 0, (PUCHAR)key, AES_KEY_SIZE, 0);
	...
	return st == 0 ? out_len + ENCRYPT_OVERHEAD : 0;
}
